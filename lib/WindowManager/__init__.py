...
class EventLoop(Thread):
    ...
    def __stopCheck__(self):
        nowTime = datetime.now() >= self.__stopAt__
        if nowTime:
...
class State:
    ...
    def __init__(self, inital=None, setHandler: Callable[[T, T], T] = None) -> None:
        """
...
class Rect:
    ...
    @property
    def center(self):
        sizedRect = self.toWidthHeight()

        return Point(
            sizedRect.x + (sizedRect.width / 2), sizedRect.y + (sizedRect.height / 2)
        )

    def toWidthHeight(self):
